...
function frame_kernel!(
	...
	x_resolution::Int64,
	y_resolution::Int64,
	result::AbstractMatrix{Int64},
	max_iter::Int=100)

    i = (blockIdx().y - 1) * blockDim().y + threadIdx().y
    j = (blockIdx().x - 1) * blockDim().x + threadIdx().x

    if i <= x_resolution && j < y_resolution
        y = y_min + (i - 1) * y_step
        x = x_min + (j - 1) * x_step
        c = complex(x, y)
...
function frame_wrapper!(sizes::window, result::AbstractMatrix{Int64}, max_iter::Int=100)
    x_step = (sizes.x_max - sizes.x_min) / (sizes.x_resolution - 1)
    y_step = (sizes.y_max - sizes.y_min) / (sizes.y_resolution - 1)

    @cuda threads=(16, 16) frame_kernel!( 
...
function zoom!(size::window, (y_mousepos, x_mousepos), rate::Float64 = 0.99, sensitivity::Float64 = 0.1)
    ...
    size.x_min = Float64(new_x_min)
    size.x_max = Float64(new_x_max)
    size.y_min = Float64(new_y_min)
    size.y_max = Float64(new_y_max)
...
function main()
    GLMakie.activate!()
    window_param = window(-1.125, 1.125, -2.5, 1.5, Int64(1080), Int64(1920))

    init_frame = CUDA.zeros(Int64, window_param.y_resolution, window_param.x_resolution)
    ...
    for _ in 1:5000
	    ...
	    zoom!(window_param, (Float64(mp[1]), Float64(mp[2])))
